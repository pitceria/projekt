    ...
    switch ($czego) {
        case 'all':
            ...
            $checks = [
                ...
                $mail_wynik->fetch_assoc()['count(email)'] != 0 => "taki adres email jest już zarejestrowany w naszym systemie spróbuj zresetować hasło",
                $nick_wynik->fetch_assoc()['count(nick)'] != 0 => "taki nickname jest już zajety prosze wybrać inny",
                $haslo_wynik->fetch_assoc()['count(haslo)'] != 0=>"takie haslo jest juz zajete",
        ...
        case 'dane':
            ...
            $sql_nick = "select count(nick) from uzytkownicy where nick like '" . $nick . "'"."and iduzytkownicy!=".$_SESSION['iduzytkownicy'];
            $nick_wynik = $conn->query($sql_nick);
